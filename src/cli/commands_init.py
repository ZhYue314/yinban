...
def init(force: bool):
    ...
    async def _run():
        ...
        if not has_key:
            ...
            click.echo(f"   ai-podcast config set ai.api_key sk-xxx")

        click.echo("\n✨ 初始化完成！试试这些命令：")
        click.echo("   ai-podcast chat            # 和AI聊天")
        click.echo("   ai-podcast recommend       # 获取推荐")
        click.echo("   ai-podcast config list     # 查看配置")
